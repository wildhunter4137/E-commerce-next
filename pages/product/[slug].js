...
export default function ProductDetail(props) {
	...
	const ratingChanged = async (productId, count) => {
		try {
			...
			toast.success('We really appreciate you taking the time to share your rating with us');
		} catch (error) {
		...
		<Layout title={product.name}>
			<div className='grid md:grid-cols-4 md:gap-3 my-2'>
				<div className='md:col-span-2 items-center'>
					<Image src={product.image} alt={product.name} width={500} height={500} className='rounded-md'></Image>
				</div>
				<div>
					<ul className='leading-9'>
						...
						<li className='whitespace-nowrap'>
							<div className='flex items-center space-x-6'>
								<ReactStars count={5} size={26} activeColor='#ffd700' value={product.rating} edit={false} />
